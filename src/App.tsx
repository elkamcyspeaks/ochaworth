...
function YellowBtn({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <button className={"bg-[#F5C842] text-[#0D2B1D] font-bold px-6 py-3 rounded-full hover:bg-[#d4a820] transition-colors " + className}>{children}</button>;
}
function GreenBtn({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <button className={"bg-[#1B3A2B] text-white font-semibold px-6 py-3 rounded-full hover:bg-[#0D2B1D] transition-colors " + className}>{children}</button>;
}
function OutlineBtn({ children, className = "", light = false }: { children: ReactNode; className?: string; light?: boolean }) {
  return <button className={"border-2 font-semibold px-6 py-3 rounded-full transition-colors " + (light ? "border-white text-white hover:bg-white hover:text-[#1B3A2B]" : "border-[#1B3A2B] text-[#1B3A2B] hover:bg-[#1B3A2B] hover:text-white") + " " + className}>{children}</button>;
}
    ...
    <div className="bg-[#0D2B1D] text-white text-xs py-2 px-4">
      <div className="max-w-7xl mx-auto flex flex-wrap justify-between items-center gap-2">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1"><MailIcon className="w-3 h-3 text-[#F5C842]"/> {site.contact.email}</span>
          <span className="flex items-center gap-1"><PhoneIcon className="w-3 h-3 text-[#F5C842]"/> {site.contact.phone}</span>
          <span className="flex items-center gap-1"><MapPinIcon className="w-3 h-3 text-[#F5C842]"/> {site.contact.topBarAddress}</span>
        </div>
        <div className="flex items-center gap-3">
          <a href={site.contact.facebook} className="hover:text-[#F5C842]"><FacebookIcon/></a>
          <a href={site.contact.twitter} className="hover:text-[#F5C842]"><TwitterIcon/></a>
          <a href={site.contact.instagram} className="hover:text-[#F5C842]"><InstagramIcon/></a>
          <a href={site.contact.youtube} className="hover:text-[#F5C842]"><YoutubeIcon/></a>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <section className="relative min-h-[90vh] flex items-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-20 w-full">
        <div className="max-w-2xl transition-all duration-500" style={{ opacity: animating ? 0 : 1, transform: animating ? "translateY(16px)" : "translateY(0)" }}>
          ...
          <h1 className="text-5xl lg:text-7xl font-black text-white leading-tight mb-6" style={{ fontFamily: "'Playfair Display', serif" }}>
            {s.line1}<br/><span className="text-[#F5C842]">{s.highlight}</span> <span className="italic">{s.italic}</span><br/>{s.line3}
          </h1>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex items-center gap-4">
        <button onClick={() => slide(-1)} className="w-10 h-10 rounded-full bg-white/20 hover:bg-[#F5C842] flex items-center justify-center text-white hover:text-[#0D2B1D]"><ChevronLeft className="w-5 h-5"/></button>
        <div className="flex gap-2">
          {heroSlides.map((_, i) => (
            <button key={i} onClick={() => { if (!animating) { setAnimating(true); setTimeout(() => { setCurrent(i); setAnimating(false); }, 400); }}} className={"transition-all duration-300 rounded-full " + (i === current ? "w-8 h-3 bg-[#F5C842]" : "w-3 h-3 bg-white/40")}/>
          ))}
        </div>
        <button onClick={() => slide(1)} className="w-10 h-10 rounded-full bg-white/20 hover:bg-[#F5C842] flex items-center justify-center text-white hover:text-[#0D2B1D]"><ChevronRight className="w-5 h-5"/></button>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-[#1B3A2B] rounded-2xl p-8 flex flex-col items-start gap-4 text-white hover:shadow-xl transition-shadow">
            <div className="w-14 h-14 bg-[#F5C842] rounded-full flex items-center justify-center"><HandIcon className="w-7 h-7 text-[#0D2B1D]"/></div>
            ...
            <button className="text-[#F5C842] font-semibold text-sm flex items-center gap-1">Join Us <ArrowRight className="w-4 h-4"/></button>
          </div>
          <div className="bg-[#F5C842] rounded-2xl p-8 flex flex-col items-start gap-4 hover:shadow-xl transition-shadow">
            <div className="w-14 h-14 bg-[#1B3A2B] rounded-full flex items-center justify-center"><GlobeIcon className="w-7 h-7 text-[#F5C842]"/></div>
            ...
            <button className="text-[#1B3A2B] font-semibold text-sm flex items-center gap-1">Get Started <ArrowRight className="w-4 h-4"/></button>
          </div>
          <div className="rounded-2xl overflow-hidden relative min-h-[280px] group">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-[#0D2B1D]/80 to-transparent flex items-end p-6">
              <div><div className="text-[#F5C842] text-xs font-bold tracking-widest uppercase mb-1">In Action</div><p className="text-white font-semibold">{site.volunteerCards.actionCaption}</p></div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="relative">
          ...
          <div className="absolute -bottom-6 left-6 bg-[#F5C842] rounded-2xl px-6 py-4 shadow-lg">
            <div className="text-3xl font-black text-[#0D2B1D]">{a.badgeNumber}</div>
            <div className="text-xs font-semibold text-[#0D2B1D]/70">{a.badgeLabel}</div>
          </div>
        </div>
        <div>
          ...
          <h2 className="text-4xl lg:text-5xl font-black text-[#0D2B1D] leading-tight mb-6" style={{ fontFamily: "'Playfair Display', serif" }}>
            {a.headingPrefix} <span className="text-[#1B3A2B]">{a.headingHighlight}</span> {a.headingSuffix}
          </h2>
          <p className="text-gray-600 leading-relaxed mb-8">{a.paragraph}</p>
          <div className="space-y-4">
            {a.features.map(f => (
              <div key={f.title} className="flex gap-4 items-start">
                <div className="w-11 h-11 bg-[#1B3A2B] text-[#F5C842] rounded-xl flex items-center justify-center flex-shrink-0"><Icon name={f.icon} className="w-5 h-5"/></div>
                <div><h4 className="font-bold text-[#0D2B1D]">{f.title}</h4><p className="text-sm text-gray-500 mt-0.5">{f.desc}</p></div>
              </div>
            ))}
          </div>
    ...
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <h2 className="text-4xl font-black text-[#0D2B1D] mt-2" style={{ fontFamily: "'Playfair Display', serif" }}>Our Journey Of <span className="text-[#1B3A2B] italic">Compassion</span></h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {site.services.map((s, i) => (
            <div key={s.title} className="group border border-gray-100 rounded-2xl p-7 hover:bg-[#1B3A2B] hover:shadow-xl transition-all duration-300 cursor-pointer">
              <div className={"w-16 h-16 rounded-xl flex items-center justify-center mb-5 transition-colors " + (i % 2 === 0 ? "bg-[#F5C842]/20 text-[#1B3A2B] group-hover:bg-[#F5C842] group-hover:text-[#0D2B1D]" : "bg-[#1B3A2B]/10 text-[#1B3A2B] group-hover:bg-[#F5C842] group-hover:text-[#0D2B1D]")}><Icon name={s.icon} className="w-10 h-10"/></div>
              ...
              <button className="mt-4 text-[#1B3A2B] group-hover:text-[#F5C842] text-sm font-semibold flex items-center gap-1">Read More <ArrowRight className="w-4 h-4"/></button>
            </div>
          ))}
        </div>
      </div>
    ...
    <section className="py-20 bg-[#F7F7F3]">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex flex-wrap justify-between items-end mb-12 gap-4">
          <div><SectionLabel text="Our Projects"/><h2 className="text-4xl font-black text-[#0D2B1D]" style={{ fontFamily: "'Playfair Display', serif" }}>Featured Projects & <span className="italic">Causes</span></h2></div>
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-10">
          <div>
            ...
            <h2 className="text-4xl lg:text-5xl font-black text-white mt-2" style={{ fontFamily: "'Playfair Display', serif" }}>Join Our <span className="text-[#F5C842]">Community</span></h2>
          ...
          <div className="text-center"><div className="text-8xl font-black text-[#F5C842]">{site.community.donorsCount}</div><div className="text-white text-lg font-semibold mt-2">Happy Donors</div></div>
        </div>
        <div className="mt-16 pt-10 border-t border-white/10">
          <p className="text-white/40 text-xs uppercase tracking-widest text-center mb-8">Trusted Partners</p>
          <div className="flex flex-wrap justify-center gap-8 items-center">
            {site.community.partners.map(p => <div key={p} className="text-white/40 font-bold text-sm tracking-widest hover:text-[#F5C842] transition-colors cursor-pointer">{p}</div>)}
          </div>
        </div>
      </div>
    </section>
...
function Volunteers() {
  return (
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-12"><SectionLabel text="Our Team"/><h2 className="text-4xl font-black text-[#0D2B1D]" style={{ fontFamily: "'Playfair Display', serif" }}>Meet Our <span className="text-[#1B3A2B] italic">Volunteers</span></h2></div>
            ...
            <div key={v.name} className="group text-center">
              <div className="relative overflow-hidden rounded-2xl mb-4">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#0D2B1D]/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                  <div className="flex gap-2">
                    <a href="#" className="w-8 h-8 bg-[#F5C842] rounded-full flex items-center justify-center"><FacebookIcon className="w-3 h-3 text-[#0D2B1D]"/></a>
                    <a href="#" className="w-8 h-8 bg-[#F5C842] rounded-full flex items-center justify-center"><TwitterIcon className="w-3 h-3 text-[#0D2B1D]"/></a>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <h2 className="text-4xl lg:text-5xl font-black text-white mt-2 leading-tight" style={{ fontFamily: "'Playfair Display', serif" }}>
              {t.headingLine1}<br/>{t.headingLine2Prefix} <span className="text-[#F5C842]">{t.headingHighlight}</span><br/>{t.headingLine3}
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {t.cards.map(c => (
              <div key={c.title} className="bg-white/10 backdrop-blur border border-white/20 rounded-2xl p-5 hover:bg-[#F5C842]/20 transition-colors">
                <div className="text-[#F5C842] mb-3"><Icon name={c.icon} className="w-7 h-7"/></div>
      ...
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="relative">
          ...
          <div className="absolute -top-6 -right-6 bg-[#F5C842] rounded-2xl px-8 py-5 shadow-xl hidden lg:block">
            <div className="text-4xl font-black text-[#0D2B1D]">{d.totalRaised}</div>
        ...
        <div>
          ...
          <h2 className="text-4xl font-black text-[#0D2B1D] mb-3" style={{ fontFamily: "'Playfair Display', serif" }}>Making A Difference,<br/>One <span className="text-[#1B3A2B] italic">Donation</span> At A Time.</h2>
          ...
          <div className="mb-5">
            ...
            <div className="flex flex-wrap gap-3 mb-3">
              {d.presetAmounts.map(a => (
                <button key={a} onClick={() => { setAmount(a); setCustom(""); }} className={"px-5 py-2 rounded-full border-2 font-bold text-sm transition-colors " + (amount === a && !custom ? "bg-[#1B3A2B] text-white border-[#1B3A2B]" : "border-gray-300 text-gray-600 hover:border-[#1B3A2B]")}>₦{a}</button>
              ))}
            </div>
            <input type="number" placeholder="Custom amount" value={custom} onChange={e => { setCustom(e.target.value); setAmount(""); }} className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#1B3A2B]"/>
          </div>
    ...
    <section className="relative py-24 overflow-hidden bg-[#0D2B1D]">
      ...
      <div className="relative max-w-4xl mx-auto px-4 text-center">
        ...
        <blockquote className="text-4xl lg:text-6xl font-black text-white leading-tight mt-4" style={{ fontFamily: "'Playfair Display', serif" }}>
          "{o.missionLine1}<br/>
          <span className="text-[#F5C842] italic">{o.missionHighlight}</span><br/>
          {o.missionLine3}"
        </blockquote>
      ...
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
        ...
        <div className="relative">
          ...
          <div className="absolute bottom-6 left-6 right-6 bg-white/90 backdrop-blur rounded-2xl p-5">
            <div className="text-[#1B3A2B] font-bold mb-1">{p.title}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div>
            ...
            <div className="flex gap-3">
              {[
                ...
                <a key={i} href={href} className="w-9 h-9 bg-white/10 rounded-full flex items-center justify-center hover:bg-[#F5C842] hover:text-[#0D2B1D] transition-colors">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-white/60">
              <li className="flex gap-2"><MapPinIcon className="w-4 h-4 text-[#F5C842] flex-shrink-0 mt-0.5"/> {c.footerAddress}</li>
              <li className="flex gap-2"><PhoneIcon className="w-4 h-4 text-[#F5C842] flex-shrink-0"/> {c.phone}</li>
              <li className="flex gap-2"><MailIcon className="w-4 h-4 text-[#F5C842] flex-shrink-0"/> {c.email}</li>
            </ul>
