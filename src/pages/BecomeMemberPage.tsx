...
import becomeMemberPageData from "@/data/becomeMemberPage.json";
import {
...
// ── Decorative member illustration ─────────────────────────────────────────
// Same cartoon-character style used on the Become A Volunteer page, with the
// chest badge relabelled for membership — decorative artwork, not page prose.
...
function MemberIllustration() {
  return (
    <svg viewBox="0 0 320 340" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="160" y="220" fontSize="6" fill="#1565C0" fontWeight="bold" textAnchor="middle">MEMBER</text>
...
// matching hidden static form ("become-a-member") in index.html.
...
function FormSection() {
  const f = becomeMemberPageData.form;
  const o = site.org;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setStatus("sending");
    try {
      await submitToNetlify("become-a-member", form);
      ...
      fetch("/.netlify/functions/notify", {
        ...
        body: JSON.stringify({ type: "become-a-member", email: form.email, name: form.name }),
      }).catch(err => console.error("Become A Member confirmation email failed:", err));
      setStatus("sent");
    ...
    <section className="py-20 bg-white">
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="flex flex-col lg:flex-row rounded-3xl overflow-hidden shadow-lg border border-gray-100">
          ...
          >
            <input type="hidden" name="form-name" value="become-a-member"/>
            <Honeypot/>
            <div className="mb-5">
...
export default function BecomeMemberPage() {
  return (
    <>
      <PageHero
        image={becomeMemberPageData.hero.image}
        title={becomeMemberPageData.hero.title}
        breadcrumbHome={becomeMemberPageData.hero.breadcrumbHome}
        breadcrumbCurrent={becomeMemberPageData.hero.breadcrumbCurrent}
      />
      <FormSection/>
      <Newsletter/>
    </>
